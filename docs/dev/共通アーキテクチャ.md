...
```
AIDraft システム群
├── 01_お問い合わせ          [Googleフォーム → スプレッドシート → AI → Gmail]
├── 02_その他セミナー        [Googleフォーム → スプレッドシート → AI → Gmail]  
├── 03_WeekendEnt           [Googleフォーム → スプレッドシート → AI → Gmail]
└── 04_Gmailラベル自動回答   [Gmail受信 → AI → Gmail返信]
```
