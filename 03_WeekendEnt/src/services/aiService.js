...
class AIService {
  ...
  static generateSeminarQuestionResponse(questionInfo) {
    ...
    const prompt = `
歯科セミナーの質問に対して、指定されたテンプレート形式で返信メールを作成してください。

【質問情報】
医院名: ${questionInfo.clinicName}
先生のお名前: ${questionInfo.doctorName}先生
セミナー感想: ${questionInfo.impression}
質問内容: ${questionInfo.question}

【返信テンプレート】
${questionInfo.clinicName}
${questionInfo.doctorName}先生
...
`;
    ...
    return this._callOpenAI(prompt, questionInfo.question);
  }
  
  static _callOpenAI(prompt, questionText) {
    const apiKey = PropertiesService.getScriptProperties().getProperty('OPENAI_API_KEY');
